...
const writeNewTaxVerificationTables = (salariesToTest)=>{
  ...
  const fs = require('fs');
  fs.writeFile(taxVerificationJSONFile, JSON.stringify(salariesToTest,null,0), 'utf8',done=>{console.log("NEW DATA WRITTEN")});
};

const readExistingTaxVerificationTables:()=>Promise<any> = ()=>new Promise((resolve,reject)=>{
  const fs = require('fs');
    ...
    describe("Test against pre-generated tables",()=>{   
      ...
      salariesToTest.incrementors.forEach((incrementorFn, index)=>{
        ...
        while(annual_income<salariesToTest.maxSal){
          ...
          if( isNaN(annual_income) || salaries.length>1 && salaries[salaries.length-1]<=salaries[salaries.length-2]){
            throw new Error(`Incrementor ${index+1} did not increment it's value on iteration - ${salaries[salaries.length-1]} - ${salaries[salaries.length-2]}`)
          }
        }
      ...
      salariesToTest.net_monthly_incomes.forEach((net_monthly_income, index)=>{
        it(`Iterator ${index}`,async()=>{
          ...
          expect(net_monthly_income).toEqual(comparisonTables.net_monthly_incomes[index]);

          console.log(`${net_monthly_income.length} table values verified for iterator ${index}` );
        })
